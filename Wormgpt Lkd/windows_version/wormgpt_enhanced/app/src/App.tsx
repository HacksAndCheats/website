...
const configuredApiBaseUrl = () =>
  cleanUrl(window.WORMGPT_CONFIG?.apiBaseUrl || import.meta.env.VITE_WORMGPT_API_URL);
const configuredWsBaseUrl = () =>
  cleanUrl(window.WORMGPT_CONFIG?.wsBaseUrl || import.meta.env.VITE_WORMGPT_WS_URL);
...
const getWsUrl = () => {
  const configured = configuredWsBaseUrl();
  if (configured) return configured;
  const loc = window.location;
...
function App() {
  ...
  const handleSendMessage = useCallback(async (content: string, isEdit = false, messageId?: string) => {
    ...
    try {
      ...
      let fullContent = '';
      // ⚡ Batch UI updates: accumulate tokens then flush to React
      // Avoids a re-render per token (was causing visible lag at high token rates)
      let pendingContent = '';
      ...
      while (true) {
        ...
        for (const rawLine of lines) {
          ...
          if (rawLine === 'data: [DONE]') { streamDone = true; break; }
          try {
            const json = JSON.parse(rawLine.slice(6));
            const delta = json.message?.content || json.response || '';
            if (delta) {
              pendingContent += delta;
              // Schedule a single RAF flush — no more than 1 re-render per frame (60fps)
              if (!rafPending) {
                rafPending = true;
